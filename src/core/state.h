...
// Forward declaration (will be implemented in crossword/crossword_game.h)
// CrosswordLevel get_crossword_level(int level);

// State initialization
static inline GameState create_game_state(const char* target_word) {
    GameState state = {0};
    ...
    // Initialize crossword level (will be set when crossword mode is entered)
    // state.crossword.current_level = get_crossword_level(1);
    
    // Initialize cursor (will be set when crossword mode is entered)
    ...
    state.crossword.show_error = 0;
    state.crossword.error_message[0] = '\0';
