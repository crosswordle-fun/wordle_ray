...
static inline int can_words_intersect(const char* word1, const char* word2) {
...
static inline int select_words_with_unique_starting_letters(char selected_words[][WORD_LENGTH + 1], int count) {
...
static inline int select_random_words_fallback(char selected_words[][WORD_LENGTH + 1], int count) {
...
static inline int select_random_words(char selected_words[][WORD_LENGTH + 1], int count) {
    if (count > MAX_CROSSWORD_WORDS) {
        count = MAX_CROSSWORD_WORDS;
    }
...
static inline int find_intersections(const char* word1, const char* word2, 
...
static inline int can_place_word(char grid[][9], int grid_width, int grid_height,
...
static inline void place_word_in_grid(char grid[][9], char word_mask[][9], int grid_width, int grid_height,
                       const char* word, int start_x, int start_y, int direction) {
    int dx = (direction == 0) ? 1 : 0;  // horizontal: move right
    int dy = (direction == 1) ? 1 : 0;  // vertical: move down
...
static inline CrosswordLevel generate_crossword(int word_count, int grid_width, int grid_height) {
