...
// (systems.h will provide the full definitions)

// Animation trigger functions
static inline void trigger_letter_pop(GameState* state, int letter_index) {
...
static inline void trigger_word_celebration(GameState* state) {
...
static inline void trigger_level_celebration(GameState* state) {
    if (!state->settings.animations_enabled) {
        return;
    }
...
static inline void trigger_letter_ease(GameState* state) {
    if (!state->settings.animations_enabled) {
        return;
    }
